...
print(device)
data_path = 'webshell_raw_data/good_bad_data.csv'
...
tokens = []
for i in contents:
    php_content = re.sub(r'//.*?(\r\n|\r|\n)', '', str(i))
    php_content = re.sub(r'#.*?(\r\n|\r|\n)', '', php_content)
    php_content = re.sub(r'\s+', ' ', str(php_content))
    # Remove multi-line comments (/* ... */)
    php_content = re.sub(r'/\*.*?\*/', '', php_content, flags=re.DOTALL)
    clean_text = php_content.replace('\n', ' ')
    pattern = r'[a-z0-9]+'
    result = re.findall(pattern, clean_text)
    tokens.append(result)

vocab = build_vocab_from_iterator(tokens, min_freq=min_freq, specials=["<unk>"])
vocab.set_default_index(vocab["<unk>"])
print(vocab(['select','php','of','this','c','add']))
...
for epoch in range(num_epochs):
    ...
    torch.save(model.state_dict(), 'webshell_lstm_model.pt')
